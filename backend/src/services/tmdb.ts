...
const CACHE_TTL = {
  SEARCH: 1800000,      // 30 minutes for search results
  ACTOR_DETAILS: 3600000,  // 1 hour for actor details
  FILMOGRAPHY: 3600000,    // 1 hour for filmography
  MOVIE_CAST: 3600000,     // 1 hour for movie cast
  MOVIE_DETAILS: 3600000,  // 1 hour for movie details
};
...
class TMDBService {
  ...
  async getActorDetails(actorId: number): Promise<Actor> {
    ...
    // Cache the result
    cache.set(cacheKey, actorData, CACHE_TTL.ACTOR_DETAILS);
    return actorData;
  }
  ...
  async getActorFilmography(actorId: number): Promise<Movie[]> {
    ...
    if (cached !== null) {
      console.log(`[Cache] Hit for filmography: ${actorId}`);
      return cached;
    }

    console.log(`[Cache] Miss for filmography: ${actorId}`);
    const response = await this.request<{
      cast: Array<{
        id: number;
        title: string;
  ...
  async getMovieDetails(movieId: number): Promise<Movie> {
    ...
    // Cache the result
    cache.set(cacheKey, movieData, CACHE_TTL.MOVIE_DETAILS);
    return movieData;
  }
}
